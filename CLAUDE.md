...
- **seats.aero Pro quota is 1000 calls/calendar day** (resets midnight UTC), tracked in the `X-RateLimit-Remaining` response header (logged per call as "quota remaining: N"). Every cron trigger polls **all** regions in `POLL_MATRIX` (one looping job, shared API key), so the floor is ~78 `/search` calls/cycle for the current 2 regions (measured: ~39/region = windows × routes × directions) plus one `/trips` call per newly-discovered leg. A cold-start cycle (empty DB) also enriches every leg — run 9813 spent 306 calls (78 search + 228 trips) bootstrapping germany-austria from scratch; steady-state `/trips` is much lower. Cron cadence must keep `cycles/day × 78` well under 1000 with headroom for `/trips`: the schedule is `0 */4 * * *` (6 cycles/day ≈ 468 search/day). Every 2h would spend ~936/day on search alone and 429 out. A 429 on any call raises `seatsaero.RateLimited`, which aborts the whole cycle (every remaining call would also 429); `run-once` exits non-zero so the CI job fails loudly instead of silently masking the lockout.
...
2. **Scheduled CI** via `.gitea/workflows/poll.yml`: cron `0 */4 * * *` (every 4 hours = 6 cycles/day; sized so the ~78 `/search` calls/cycle across all regions stay well under the seats.aero 1000-calls/day cap with headroom for `/trips` enrichment — see the quota invariant above). A **single `poll` job loops over every region** in the `POLL_MATRIX` Actions variable (read via `env:` + `jq -c '.[]'`, so the region list stays out of git). It is deliberately NOT a `strategy.matrix`: Gitea (verified through 1.26.2 + act_runner 0.6.x) freezes `strategy.matrix` at plan time before any source is populated — `vars` is not injected into `strategy.matrix` at all, and a `needs.<job>.outputs` value isn't ready yet — so a dynamic `fromJSON(...)` matrix collapses to a single empty job (Gitea #25179; the debug step showed `toJSON(matrix)` = `{}` while the needs output was correctly populated). The loop uses process substitution (`done < <(... jq ...)`), not a pipe, so the `fail` flag survives the loop body; each region runs in a `( set -e ... )` subshell so one region's failure sets `fail=1` and continues to the next, and the step exits non-zero if any region failed. Each entry is `{ name, search, db }`; per region the MinIO `mc` client (aliased as `homelab` against `S3_ENDPOINT`) pulls `<search>` (renamed to `config/search.yaml`) + the shared `balances.yaml` + `<db>`, runs `init-db` + `run-once` with `RESEARCHER_DB_PATH=data/<db>`, then pushes the DB back. `jq` and `mc` are installed at job start (the `node:20-bookworm` runner image has neither). Job-level concurrency `group: poll` (cancel-in-progress false) serializes overlapping runs so they don't race the shared DBs. Required Actions variables: `S3_ENDPOINT`, `S3_BUCKET`, `S3_PREFIX` (defaults to `award-flight-researcher`), `POLL_MATRIX` (JSON array). Required secrets: `SEATSAERO_API_KEY`, `PUSHOVER_TOKEN`, `PUSHOVER_USER`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`. The CI path wires Pushover only — `NTFY_*` env vars are not passed through, so deployed alerts go via Pushover even though the code supports both. Each region has its own DB and alert stream — `balances.yaml` is shared, so multi-region setups assume balance pools don't need to be partitioned across regions.
