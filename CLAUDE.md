...
2. **Scheduled CI** via `.gitea/workflows/poll.yml`: cron `0 */2 * * *` (every 2 hours ≈ 12 cycles/day per region, sized to stay under the seats.aero 1000-calls/day cap with headroom for `/trips` enrichment). Runs one matrix job per region. Matrix entries come from a `setup` job that passes through the `POLL_MATRIX` Actions variable via `region: ${{ fromJSON(needs.setup.outputs.matrix) }}` (so the region list isn't committed). Two non-obvious constraints: (1) `setup` must emit the value via `env:` + `echo "matrix=$POLL_MATRIX"`, not inline `${{ vars.POLL_MATRIX }}` interpolation, or the shell strips the JSON's double quotes into invalid JSON; (2) the matrix cannot read `vars.POLL_MATRIX` directly — `vars` is not available in `strategy.matrix` (expanded server-side before `vars` injection), so it must round-trip through `needs.<job>.outputs`. Dynamic matrices require **Gitea >= 1.26** (older servers collapse a dynamic matrix to a single empty job — Gitea #25179 / PR #32823). Each entry is `{ name, search, db }`; the job uses the MinIO `mc` client (aliased as `homelab` against `S3_ENDPOINT`) to pull `<search>` (renamed to `config/search.yaml` for the run) + the shared `balances.yaml` + `<db>` from MinIO, runs `init-db` + `run-once` with `RESEARCHER_DB_PATH=data/<db>`, then pushes the DB back. Concurrency is scoped per region (`poll-<name>`), so regions run in parallel without racing each other's DB. Required Actions variables: `S3_ENDPOINT`, `S3_BUCKET`, `S3_PREFIX` (defaults to `award-flight-researcher`), `POLL_MATRIX` (JSON array). Required secrets: `SEATSAERO_API_KEY`, `PUSHOVER_TOKEN`, `PUSHOVER_USER`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`. The CI path wires Pushover only — `NTFY_*` env vars are not passed through, so deployed alerts go via Pushover even though the code supports both. Each region has its own DB and alert stream — `balances.yaml` is shared, so multi-region setups assume balance pools don't need to be partitioned across regions.
